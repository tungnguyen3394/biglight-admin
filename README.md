...
- GAS は **v3**（CRM と同じ: senderName 対応・doGet で「v3」表示・sendTest）。差出人名 = ログイン名そのまま（会社名の前置きなし）。旧版の人は v3 を貼り直して「新しいデプロイ」→ 新 URL を登録。サーバは GAS の  /  どちらも失敗として扱う。
- 添付ファイルは （公開 URL だが推測不可）。旧形式  は起動時に自動リネーム。
